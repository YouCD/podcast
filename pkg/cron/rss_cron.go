...
// StartRSSCronJob 启动定时爬取RSS任务
func StartRSSCronJob(ctx context.Context) {
	// 获取应用配置中的RSS配置
	c := cron.New()
	// 创建处理器
	workFlow, err := workflow.New(ctx)
	if err != nil {
	...
	_, _ = c.AddFunc("*/10 * * * *", func() {
		...
		_, err := workFlow.Invoke(ctx, config.Cfg.RSS)
		if err != nil {
	...
	go func() {
		...
		output, err := workFlow.Invoke(ctx, config.Cfg.RSS)
		if err != nil {
...
func StartReportJob(ctx context.Context) {
	...
	for _, r := range config.Cfg.Report {
		// 每小时执行一次RSS爬取任务
