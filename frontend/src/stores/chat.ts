...
export const useChatRecordsStore = defineStore("chatRecords", () => {
    ...
    try {
      ...
      const parsedMessages: ParsedMessage[] = resp.messages.map((message) => {
        ...
        if (typeof stepsRaw === "string" && stepsRaw) {
          ...
          console.log(
            "Steps is string, trimmed length:",
            trimmed.length,
            "first 50 chars:",
          ...
          console.log(
            "Steps is not a string or is falsy:",
            typeof stepsRaw,
            !!stepsRaw,
          );
        }
  ...
  const SetReasoningTyping = (uuid: string) => {
  ...
  const setPlan = (uuid: string, plan: any) => {
    if (state.value.sessionData) {
      const message = state.value.sessionData.messages.find(
        (m) => m.uuid === uuid,
      );
      if (message) {
  ...
  const setStepResult = (
    ...
    console.log("setStepResult called:", {
      uuid,
      stepId,
    ...
    if (state.value.sessionData) {
      ...
      console.log(
        "setStepResult: message.steps?",
        message?.steps
          ? JSON.stringify(message.steps).substring(0, 200)
          : "undefined",
      );
      if (message && message.steps) {
        ...
        if (step) {
          ...
          console.log(
            "setStepResult: step updated",
            JSON.stringify(step).substring(0, 200),
          );
        }
      }
    }
