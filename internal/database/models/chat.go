...
type Message struct {
	...
	// 新增字段：计划数据（JSON格式）
	Plan string `gorm:"type:text;comment:计划数据" json:"plan"`
	// 新增字段：步骤数据（JSON格式）
	Steps string `gorm:"type:text;comment:步骤数据" json:"steps"`
	// 新增字段：思考内容
	ThinkContent string    `gorm:"type:text;comment:思考内容" json:"think_content"`
	CreatedAt    time.Time `gorm:"type:timestamp;not null" json:"created_at"`
}
