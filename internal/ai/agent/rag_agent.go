...
func RunRagAgent(ctx context.Context, messages []*schema.Message, opt ...agent.AgentOption) (*schema.StreamReader[*schema.Message], error) {
	// 初始化LLM
	tools, err := initMcpTool(ctx)
	if err != nil {
		return nil, err
	}
	...
	//}, einoAgent.WithComposeOptions(compose.WithCallbacks(&AgentCallback{})))
	//
	//fmt.Println(generate.Content)
	//return nil, nil

	return agent.Stream(ctx, messages, opt...)
}
