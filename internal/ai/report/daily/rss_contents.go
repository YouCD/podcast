...
func rssContents(ctx context.Context, state *graphState) (*graphState, error) {
	...
	log.WithCtx(ctx).Infow("rssContents", "时间范围", fmt.Sprintf("开始获取RSS内容 %s~%s", state.startDate, state.endDate))
	// 获取昨天的所有RSS内容
	...
	if len(rssContentList) == 0 {
		log.WithCtx(ctx).Infow("rssContents", "时间范围", fmt.Sprintf("%s~%s", state.startDate, state.endDate), "RSS总数", 0)
